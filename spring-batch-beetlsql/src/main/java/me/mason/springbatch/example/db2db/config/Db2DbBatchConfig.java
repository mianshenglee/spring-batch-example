...
public class Db2DbBatchConfig {
    ...
    public ItemReader db2DbItemReader(@Qualifier("originDatasource") DataSource originDatasource) {
        JdbcCursorItemReader<User> jdbcCursorItemReader = new JdbcCursorItemReader<User>();
        jdbcCursorItemReader.setDataSource(originDatasource);
        jdbcCursorItemReader.setSql(" select * from test_user");
